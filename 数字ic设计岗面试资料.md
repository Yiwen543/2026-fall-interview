...
# 补充章节：BTB 判断依据与分支预测器汇总

---

## 2.9 BTB（分支目标缓冲器）判断依据与流程

这个双发 2-Way Gshare 分支预测器通过整合 **GHR（全局历史寄存器）**、**BTB（分支目标缓冲器）** 和 **BHT（分支历史表）**，在单周期内同时对两条指令完成方向与目标地址的推测。

**1. Gshare 的工作原理与索引机制**

* **全局历史追踪**：硬件内部维护一个 `ghr` 寄存器，记录最近执行分支的跳转历史（1 代表跳，0 代表不跳）。


* **异或索引生成**：查表索引通过将指令 PC 的中间有效位与 GHR 进行按位异或生成（如 `fetch_idx_0 = fetch_pc_0[IDX_BITS+1:2] ^ out_pred_ghr_0`）。这种哈希方式利用历史上下文打散了单靠 PC 索引容易造成的表项别名冲突（Aliasing）。


* **双发连续推测**：对于同一拍取的第二条指令，若第一条指令是分支，则将第一条的推测结果拼接进历史（`out_pred_ghr_1 = {ghr[IDX_BITS-2:0], pred_taken_0}`），确保第二条指令基于最新的推测历史计算索引。



**2. BTB 判断分支存在与跳转地址**

* **表项字段**：BTB 存储了有效位（`btb_valid`）、高位标签（`btb_tag`）以及上一次实际跳转的目标 PC（`btb`）。


* **命中判定（BTB Hit）**：通过比较当前 PC 高位标签与表项内存储的 Tag，且满足 `valid=1`，即判定命中（`btb_hit_0 = btb_valid[idx] && (btb_tag[idx] == tag)`）。


* **目标地址给出**：只要确认为分支指令且 BTB 命中，直接输出表项中保存的预测目标地址 `pred_target_0 = btb[fetch_idx_0]`。



**3. BHT 与最终跳转方向判定**

* **2-bit 饱和计数器**：BHT 数组中每一项存储 2 位状态（`11` 强跳、`10` 弱跳、`01` 弱不跳、`00` 强不跳），以最高位（MSB）为 1 代表预测跳转。


* **三重条件与门控**：最终的跳转预测（`pred_taken_0`）必须同时满足三个条件：
1. 指令本身确认为分支（`is_branch_inst_0`）；


2. BTB 匹配命中（`btb_hit_0`）；


3. BHT 计数器最高位为 1（`bht[fetch_idx_0][1] == 1'b1`）。




* 这种机制避免了非分支指令因 Gshare 哈希碰撞误匹配到陈旧 BTB 表项而导致取指走入歧途。



**4. Execute 阶段的更新与恢复**

* **方向纠正与学习**：分支执行完成后，若实际发生跳转，BHT 计数器向上饱和累加并更新 BTB 的目标地址与 Tag；若不跳转，则 BHT 计数器向下递减。


* **误预测历史回退**：当执行阶段发现预测错误（`actual_taken != update_pred_taken`）时，硬件放弃推测的 GHR，直接使用执行阶段快照的 `update_ghr` 拼接实际方向进行精准恢复。

---

## 8.2 除 gshare 外的主流分支预测器汇总

> **演进核心**：解决别名冲突（Aliasing），捕捉更长、更复杂的代码历史模式。

### 1. 基础/经典预测器
* **Bimodal (双峰预测器)**：只用 PC 低位索引 PHT（存 2-bit 饱和计数器）。最简单、延迟最低，但无历史信息，别名冲突严重。
* **gselect (全局选择)**：将 **PC 部分 bit** 与 **GHR（全局历史）** 直接**拼接**去查 PHT（gshare 采用的是**异或 $\text{XOR}$**）。
* **PShare (局域历史预测器)**：$\text{PC} \rightarrow \text{LHT (局域历史表)} \rightarrow \text{PHT}$。专门捕捉单个分支自身强周期性规律（如 `for` 循环）。

### 2. 组合/现代 SOTA 预测器
* **Tournament (锦标赛预测器)**：同时例化 Global 和 Local 预测器，中间用 **Meta 选择器** 根据历史表现动态决定采纳哪方的结果（代表作：Alpha 21264）。
* **TAGE (带有 Tag 的几何级数历史预测器)**：
  * **工业界标配/SOTA**（Intel Core, AMD Zen, 香山 RISC-V 均采用）；
  * 由多个带 Tag 的预测表组成，历史长度按几何级数增长（几个 bit 到数百 bit）；
  * **匹配原则**：选择 **Tag 命中且历史最长的那个表（Provider Table）** 作为预测输出，彻底消除别名冲突。
* **Perceptron (感知机预测器)**：引入机器学习单层神经网络，利用权重向量与 GHR 相乘求和。资源随历史长度呈**线性增长 $O(N)$**，能以极小面积支持超长历史。

---

### 💡 面试速记表

| 预测器类型 | 索引 / 查询依据 | 核心优势 | 缺点 / 局限 |
| :--- | :--- | :--- | :--- |
| **Bimodal** | PC | 逻辑最简单，延迟最低 | 无历史，冲突严重 |
| **gshare** | $\text{PC} \oplus \text{GHR}$ | 利用全局相关性，消除部分冲突 | 历史长度受限（表呈 $O(2^N)$ 指数增长）|
| **PShare** | PC $\rightarrow$ LHT | 极其擅长单分支强周期循环 | 无法捕捉跨分支影响 |
| **TAGE** | 多 Tag 表 + 几何级历史 | **当前准确率最高（SOTA）**，消除冲突 | 更新策略与控制逻辑繁琐 |
| **Perceptron** | 权重向量 $\cdot$ GHR | **线性面积资源 $O(N)$** 支持超长历史 | 求和加法树导致组合延迟较大 |
...
m_araddr     : ------< 32'h0000_1000 >------------------------


m_arlen      : ------<      4'd3     >------------------------ (len=3 表示 4 拍)


m_arsize     : ------<      3'b101   >------------------------ (2^5 = 32 Bytes = 256 bits)


m_arburst    : ------<     2'b01     >------------------------ (INCR 递增突发)


m_arvalid    : ______/‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾\________________________


s_arready    : ____________/‾‾‾‾‾‾‾‾‾\________________________
               (握手成功: T2 拍)
...
s_rvalid     : ____________/‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾\______


s_rdata[255:0]: -----------< D0 ><  D1  ><  D2  ><  D3  >-----


s_rlast      : _________________________________/‾‾‾‾‾\_______


s_rresp[1:0] : ------------< OK ><  OK  ><  OK  ><  OK  >----- (2'b00 = OKAY)


                               |        |        |        |
                         数据拍 1    数据拍 2    数据拍 3    数据拍 4
                        (握手成功)  (握手成功)  (握手成功)  (最后一拍+RLAST)
