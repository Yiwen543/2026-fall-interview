...
## 8.1 BTB（分支目标缓冲器）判断依据与流程

> **核心逻辑**：取指（Fetch）阶段以 **当前 PC** 为唯一查询依据，同时回答“是不是跳转指令”和“跳到哪里”。

### 1. 硬件判断流程（以 PC 为索引）
* **Index 索引**：用当前 PC 的低位（截去下对齐位，如 `PC[11:2]`）定位 BTB 表项（Entry）。
* **Tag 比对**：将表项内保存的 Tag 与当前 PC 的高位比对：
  * **Hit（命中）**：Tag 匹配且 Valid=1 $\rightarrow$ 确定是跳转指令，输出缓存的 **Target PC**；
  * **Miss（未命中）**：Tag 不匹配或 Valid=0 $\rightarrow$ 判为普通顺序指令（`PC + 4`），或未记录的新跳转。
* **目标重定向**：若 BTB Hit，且方向预测器（BHT）预测为 **Taken**，下一个周期 PC 直接重定向至 **Target PC**。

### 2. BTB 表项（Entry）关键字段
| 字段 (Field) | 作用 / 判断依据 |
| :--- | :--- |
| **Valid Bit** | 标定该表项是否有效。 |
| **Tag** | 与当前 PC 高位比对，防止不同 PC 发生别名冲突（Aliasing）。 |
| **Target PC** | 记录该指令上一次跳转的目标地址。 |
| **Branch Type (可选)** | 标识指令类型（JAL / JALR / Branch / RET），协助调用 RAS。 |
